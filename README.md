...
### Защита от пустого прайса
Если в «Входні товари» оказалось меньше **50 %** товаров от числа строк «Усі товари»
(лист пуст, ещё грузится или его чистят), синхронизация **прерывается**: наличие не
трогается, в «Лог» пишется строка `sync-ОСТАНОВЛЕН` с причиной. Без этой защиты
прогон по пустому прайсу помечает весь каталог «Немає в наявності» (так уже случалось).
Если обнуление действительно нужно — пункт меню «⚠ Синхронізація без захисту (force)»
или `force = true` при ручном запуске Action.
